...
class SoundLogger_cfm(Callback):
    def __init__(self, train_batch_frequency,val_batch_frequency, max_sound_num, sr=22050, clamp=True, increase_log_steps=False,
                 rescale=True, disabled=False, log_on_batch_idx=False, log_first_step=False, cfm_steps=20, size_len=64, guidance_scale=1.0, uncond_cond=None, fps=21.5):
        super().__init__()
        self.fps = fps
        self.rescale = rescale
        self.train_batch_freq = train_batch_frequency
    ...
    def save_waveform(self, savepath, waveform, sr):
    ...
    @rank_zero_only
    def log_local(self, save_dir, split, log_dict,
                  global_step, current_epoch, batch_idx, pl_module, show_curve=False, scale_factor=1):

        # root:
        root = os.path.join(save_dir, "sound_eval", split, "epoch_{}_step_{}".format(current_epoch, global_step))
        self.inverse_op = pl_module.inverse_op
        ...
        for i in range(len(gt_sound_list)):
            ...
            if mix_info_dict['audio_name2'][i] == "":
                ...
                gt_sound = self.inverse_op(gt_sound_list[i])
                rec_sound = self.inverse_op(rec_sound_list[i])
                self.save_waveform(os.path.join(sample_folder, "sample_{}_gt.wav".format(i)), gt_sound, self.sr)
                self.save_waveform(os.path.join(sample_folder, "sample_{}_stage1_rec_clamp.wav".format(i)), rec_sound, self.sr)
                sample = self.inverse_op(diff_sample_list[i])
                self.save_waveform(os.path.join(sample_folder, "sample_{}_diff_sample_clamp.wav".format(i)), sample, self.sr) 
                ...
                gt_sound = self.inverse_op(gt_sound_list[i])
                rec_sound = self.inverse_op(rec_sound_list[i])
                self.save_waveform(os.path.join(sample_folder, "sample_{}_concat_gt.wav".format(i)), gt_sound, self.sr)
                self.save_waveform(os.path.join(sample_folder, "sample_{}_stage1_concat_rec_clamp.wav".format(i)), rec_sound, self.sr)
                sample = self.inverse_op(diff_sample_list[i])
                self.save_waveform(os.path.join(sample_folder, "sample_{}_diff_concat_sample_clamp.wav".format(i)), sample, self.sr) 
    ...
    def log_sound_steps(self, pl_module, batch, batch_idx, split="train"):        
        ...
        with torch.no_grad():
            log_dict = pl_module.log_sound_cfm(batch,N=self.max_sound_num, cfm_steps=self.cfm_steps, split=split, size_len=self.size_len, guidance_scale=self.guidance_scale, uncond_cond=self.uncond_cond)
        ...
        self.log_local(pl_module.logger.save_dir, split, log_dict, pl_module.global_step, pl_module.current_epoch, batch_idx, pl_module)
...
class SoundLogger_concat_fullset(Callback):
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        # if not self.disabled and (pl_module.global_step % self.val_vis_frequency == 0) and pl_module.global_step > 0:
        if not self.disabled and batch_idx == 0 and (pl_module.current_epoch >= 80) and (pl_module.current_epoch % 4 == 0):
            self.log_sound_steps(pl_module, batch, batch_idx, split="val")
            # pass
