...
class audio_video_spec_fullset_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, dataset1, feat_type='clip', transforms=None, sr=22050, duration=10, truncate=220000, fps=21.5, debug_num=False, fix_frames=True, hop_len=160):
        super().__init__()
    ...
    def load_spec_and_feat(self, spec_path, video_feat_path):
        ...
        # Padding the samples:
        spec_len = self.sr * self.duration / self.hop_len # 16000*10/160 = 1000
        assert spec_raw.shape[-1] == 64
        if spec_raw.shape[0] < spec_len:
            temp_spec = np.zeros((spec_len,spec_raw.shape[-1]), dtype=np.float32)
            temp_spec[:spec_raw.shape[0],:] = spec_raw
        spec_raw = spec_raw[:int(spec_len),:]
        
        feat_len = self.fps * self.duration # 4*10 = 40
    ...
    def mix_audio_and_feat(self, spec1=None, spec2=None, video_feat1=None, video_feat2=None, video_info_dict={}, mode='single'):
        ...
        if mode == "single":
            ...
            if not self.fix_frames:
                start_idx = random.randint(0, self.sr * self.duration - self.truncate - 1) 
            else:
                start_idx = 0 # audio start

            start_frame = int(self.fps * start_idx / self.sr) # 0
            truncate_frame = int(self.fps * self.truncate / self.sr) # 40

            # Spec Start & Truncate:
            spec_start = int(start_idx / self.hop_len) # 0
            spec_truncate = int(self.truncate / self.hop_len) # 1000

            spec1 = spec1[spec_start : spec_start + spec_truncate, :] # [0:1000, :]
            video_feat1 = video_feat1[start_frame: start_frame + truncate_frame] # [0:40]
            ...
            total_spec_len = int(self.truncate / self.hop_len) # 1000
            # Random Trucate len:
            spec1_truncate_len = random.randint(self.min_duration * self.sr // self.hop_len, total_spec_len - self.min_duration * self.sr // self.hop_len - 1) # randint(20,1000-20-1) 
            ...
            concat_audio_spec = np.concatenate([spec1, spec2], axis=0)  
    ...
    def __getitem__(self, idx):
        ...
        data_dict = {}
        data_dict['mix_spec'] = mix_spec[None]
