...
class EvalDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        example_feat = np.load(feat_path)['feat'].astype(np.float32)
...
if __name__ == "__main__":
    ...
    config = '/workspace/data_dir/tmp_dyh/Diff-Foley/training/neuroband/configs/evaluations/audioldm_large_cfm.yaml'
    ...
    model.eval()
    # benchmark eval
    feat_folder = '/workspace/data_dir/USB_data/dyh_data/MUSIC_dataset_feat'
    steps = 50
    cfg_scale = 2
    repeat_times = 1
    pred_folder = f'/workspace/data_dir/USB_data/dyh_data/MUSIC_dataset_our_cfm_pred_cfg_{cfg_scale}_step_{steps}_repeat_{repeat_times}'
    ...
    # # single_test
    # example_feat = np.load('/workspace/data_dir/tmp_dyh/vgg_sound/video_feat/_2ZnlzncI3k_000070.npz')['arr_0'].astype(np.float32)
    # example_feat = torch.from_numpy(example_feat)[0:39,:].unsqueeze(0).to(device)
    # cond = model.get_learned_conditioning(example_feat)
    # un_cond = torch.zeros(cond.shape).to(device)
    # steps = 25
    # cfg_scale = 1.5
    # audio_samples, _ = model.sample_log_diff_sampler(cond, batch_size=1, sampler_name='DPM_Solver', ddim_steps=steps, unconditional_guidance_scale=cfg_scale,unconditional_conditioning=un_cond)
    # # audio_samples = model.sample_cfm(cond,batch_size=1, cfm_steps=steps, size_len=248, unconditional_guidance_scale=cfg_scale,unconditional_conditioning=un_cond)
    # audio_samples = model.decode_first_stage(audio_samples)
    # # draw_mel_spec(audio_samples[0][0])
    
    

    # audio_samples = model.inverse_op(audio_samples)
    # save_waveform('/workspace/data_dir/tmp_dyh/Diff-Foley/training/neuroband/eval_result/test.wav',audio_samples,16000)
    # merge_video_audio('/workspace/data_dir/tmp_dyh/Diff-Foley/training/neuroband/eval_result/origin.mp4','/workspace/data_dir/tmp_dyh/Diff-Foley/training/neuroband/eval_result/test.wav',f'/workspace/data_dir/tmp_dyh/Diff-Foley/training/neuroband/eval_result/output_{seed}_step_{steps}_cfg_{cfg_scale}.mp4')
