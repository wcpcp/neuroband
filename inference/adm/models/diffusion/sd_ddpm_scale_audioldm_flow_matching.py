...
import os
from adm.util import log_txt_as_img, exists, default, ismap, isimage, mean_flat, count_params, instantiate_from_config
from adm.modules.ema import LitEma
from audioldm_train.modules.diffusionmodules.distributions import DiagonalGaussianDistribution
from adm.models.autoencoder_img import AutoencoderKL
...
from audioldm_train.utilities.model_util import instantiate_from_config as instantiate_first_stage_config
# Add Other Sampler:
from adm.models.diffusion.plms import PLMSSampler
from adm.models.diffusion.dpm_solver import DPMSolverSampler
...
class DDPM(pl.LightningModule):
    ...
    def q_sample(self, x_start, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        t_rs = t.detach().clone()
        t_rs = t.view(x_start.shape[0],1,1,1).to(self.device)
        z = (1. - t_rs) * noise + t_rs * x_start
        return z
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.sample_t_m = -0.5
        self.sample_t_s = 1.0
        # timesteps = torch.sigmoid(torch.randn((kwargs['timesteps'],)))
    ...
    def instantiate_first_stage(self, config):
        model = instantiate_first_stage_config(config)
    ...
    def get_first_stage_encoding(self, encoder_posterior):
        ...
        z = (z + self.mean_factor)*self.scale_factor
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                ...
                c = self.cond_stage_model(c) # [b, 40, 768]
        else:
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['caption', 'coordinates_bbox']:
                    ...
                    xc = super().get_input(batch, cond_key).to(self.device) # cond
            else:
                xc = x
    ...
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False):
        ...
        z = 1. / self.scale_factor * z - self.mean_factor
    ...
    #             c = self.get_learned_conditioning(c) # [b,40,768]
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        # t = torch.randint(0,1000,(x.shape[0],),device=self.device)/1000
        
        t = torch.normal(self.sample_t_m,self.sample_t_s,(x.shape[0],),device=self.device)
        t = torch.sigmoid(t)
        # self.timesteps = self.timesteps.to(self.device)
        # timestep_indices = torch.randint(1,999,(x.shape[0],),device=self.device)
        # t = [self.timesteps[index.item()] for index in timestep_indices]
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        model_output = self.apply_model(z_t, t, cond)
        
        t_rs = t.detach().clone().view(model_output.shape[0],1,1,1)
        noise_pred = x_start- noise #(1 - t_rs) * model_output + z_t
        loss_dict = {}
        prefix = 'train' if self.training else 'val'
        ...
        #     raise NotImplementedError()
        target = model_output
        loss_cfm =self.get_loss(noise_pred, target, mean=False).mean([1, 2, 3])
        
        loss_dict.update({f'{prefix}/loss_cfm': loss_cfm.mean().item()})
        ...
        loss = self.l_simple_weight * loss_cfm.mean()
    ...
    def check_first_stage(self,x):
        z = self.enc(x)
        print(z[1])
        rec_mel = self.decode_first_stage(z)
        print(rec_mel[1])
        print(rec_mel.shape)
        waveform = self.mel_spectrogram_to_waveform(rec_mel,savepath='/mnt/petrelfs/tangshixiang/dyh/yt8m_scripts/log')
    ...
    def save_waveform(self, waveform, savepath, name="outwav"):
        for i in range(waveform.shape[0]):
            ...
            sf.write(path, todo_waveform, samplerate=16000)
    ...
    @torch.no_grad()
    def sample_log(self,cond,batch_size, ddim, ddim_steps, size_len=250, unconditional_guidance_scale=1.0,unconditional_conditioning=None, **kwargs):

        if ddim:
            ddim_sampler = DDIMSampler(self)
            # shape = (self.channels, self.image_size, self.image_size)
            # shape = (self.channels, 16, size_len)
            shape = (self.channels, size_len, 16)
    ...
    @torch.no_grad()
    def sample_cfm(self,cond,batch_size, cfm_steps, size_len=250, unconditional_guidance_scale=1.0,unconditional_conditioning=None, **kwargs):
        
        shape = (batch_size, self.channels, size_len, 16)
        z_0 = torch.randn(shape, device=self.device)
        z = z_0.detach().clone()
        # self.timesteps = self.timesteps.to(self.device)
        # sigmas = torch.cat([self.timesteps,torch.zeros(1,device=self.device)])
        # num_train_step = self.timesteps.shape[0]
        for i in range(cfm_steps):
            # sigma_i = i*int(num_train_step/cfm_steps)
            # sigma_i_next = (i+1)*int(num_train_step/cfm_steps)
            # dt = sigmas[sigma_i_next] - sigmas[sigma_i]
            dt = 1.0/cfm_steps
            # print(f"step{i+1}, sigma_i:{sigma_i}, sigma_i_next:{sigma_i_next}, dt:{dt}")
            # dt = -1./cfm_steps
            t = torch.ones((batch_size,), device=self.device) * i/cfm_steps
            z_in = torch.cat([z] * 2)
            t_in = torch.cat([t] * 2)
            c_in = torch.cat([unconditional_conditioning,cond])
            neg_pred,pos_pred = self.apply_model(z_in,t_in,c_in).chunk(2)
            pred = neg_pred + unconditional_guidance_scale * (pos_pred - neg_pred)
    ...
    @torch.no_grad()
    def log_sound_cfm(self, batch, N=4, n_row=4, sample=True, cfm_steps=20, return_keys=None,
        ...
        if sample:
            with self.ema_scope("Plotting"):
                ...
                samples = self.sample_cfm(cond=c, batch_size=N, cfm_steps=cfm_steps, size_len=size_len, unconditional_guidance_scale=guidance_scale,unconditional_conditioning=uncond_cond)
